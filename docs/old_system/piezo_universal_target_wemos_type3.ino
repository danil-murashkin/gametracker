...
#define NEOQTY 5  //One Neopixel is optional, externally attached through minijack
#define EEPROMSIZE 128

unsigned int hits = 0;
unsigned int maxhits = 3;
String ssid = "TARGET-DEFAULT";
String pass = "targetpass";
String expssid = "TARGET-EXPLOSION";
byte power = 10;
byte exppower = 20;
byte chan = 4;
...
unsigned int ExplosionTimeout = 500;  // Explosion time after death
...
bool ExplosionTimeActive = false;
unsigned long ExplosionTimeStart = 0;
...
void loop() {
  if (hitDetected && hits < maxhits) {
    ...
    //delay(50);
    if (hits >= maxhits) {
      Serial.println("MAX HITS REACHED - TARGET DEAD!");
      if (expssid != "") Explosion();
      Die();
    } else {
      HitTimeActive = true;
      HitTimeStart = 0;
      if (HitRegenTimeoutSec > 0) {
...
void ScanCheck() {
  if (expssid == "") return;  // Do not detect PA signals if no explosion configured
  int n = WiFi.scanComplete();
  if (n >= 0) {
    for (int i = 0; i < n; i++) {
      if (WiFi.SSID(i) == "EMPBOMB-B" && hits < maxhits) {
        Serial.println("EMP BOMB DETECTED - BOOM!");
        Explosion();
        hits = maxhits;
        Die();
      }
      if (WiFi.SSID(i) == "REPDEV-A" && hits < maxhits && hits > 0) {
        Repair();
      }
      if (WiFi.SSID(i) == "RESDEV-C" && hits >= maxhits) {
        Resurrect();
      }
    }
    WiFi.scanDelete();
...
void Explosion() {
  server.stop();
  WiFi.softAP(expssid.c_str(), pass.c_str(), chan, false, 1);
  WiFi.setOutputPower(float(exppower));
  delay(20);
  ExplosionTimeActive = true;
  ExplosionTimeStart = 0;
}
...
void TimersCheck() {
  ...
  if (ExplosionTimeActive) {
    if (ExplosionTimeStart == 0) ExplosionTimeStart = millis();
    if (ExplosionTimeout <= (millis() - ExplosionTimeStart)) {
      ExplosionTimeActive = false;
      ExplosionTimeStart = 0;
      StartAPScan();
    }
  }
...
void ProcessCommands() {
  if (inputStr.substring(0, 4) == "test") {
    ...
    EEPROM.commit();
  } else if (inputStr.substring(0, 7) == "expssid") {
    inputStr.remove(0, 8);
    Serial.println("expssid configured");
    expssid = inputStr;
    WriteString(expssid, 73, 104, true);
  } else if (inputStr.substring(0, 8) == "exppower") {
    inputStr.remove(0, 9);
    Serial.println("exppower configured");
    exppower = byte(inputStr.toInt());
    EEPROM.write(105, exppower);
    EEPROM.commit();
...
void SaveAllConfig() {
  ...
  WriteString(expssid, 73, 104, false);
  EEPROM.write(105, exppower);
  EEPROM.commit();
}
...
void ReadAllConfig() {
  ...
  expssid = ReadString(73, 104);
  exppower = EEPROM.read(105);
}
...
void SendAllConfig() {
  ...
  delay(n);
  SendStringParam("expssid", expssid, 31);
  delay(n);
  SendByteParam("exppower", exppower, 0, 20);
  delay(n);
  SendCmdParam("APPLY");
  delay(n);
...
void ShowConfig() {
  ...
  Serial.print("expssid = ");
  Serial.println(expssid);
  Serial.print("exppower = ");
  Serial.println(exppower);
}

void ShowHelp() {
  Serial.println("info - display system information");
  Serial.println("flush - clear EEPROM");
  Serial.println("save - save current config");
  Serial.println("defaults - reset all config values to defaults (don't forget to save)");
  Serial.println("show - show current config");
  Serial.println("reboot - guess what?");
  Serial.println("hits - set current hits");
  Serial.println("maxhits - set max hits");
  Serial.println("hit_regen_time - hit regeneration time in seconds. Set to 0 to disable.");
  Serial.println("resurrect_time - resurrection time in second. Set to 0 to disable");
  Serial.println("ssid - WiFi SSID during normal operation");
  Serial.println("pass - WiFi password");
  Serial.println("power - WiFi power in dBm, 0 to 20");
  Serial.println("expssid - explosion SSID. Set to empty string to disable powerarmor explosion and scan signals");
  Serial.println("exppower - explosion power in dBm, 0 to 20");
  Serial.println("help - displays this text");
  Serial.println("LIVE - immediately resurrect target");
  Serial.println("APPLY - immediately apply current settings");
}
